...
// 初始化：绑定 CommonUser 的登录完成事件与会话子系统的 PreClientTravel 事件
...
// 取主玩家的 PlayerController（第一个本地玩家的）
...
// 是否允许加入被请求的会话：通常要等本局流程已经可以进人了才返回 true
...
// 用户登录结果回调：成功后把玩家送进被请求的会话，失败则给出提示
...
// 收到服务端下发的加密令牌：本地生成/保存密钥并回执（示例实现，非安全方案）
...
// 客户端 Travel 之前：往 URL 上追加本局需要的额外参数

...
// 填充一个类的复制信息：是否空间化会决定剔除距离、更新频率等参数
...
// 按需初始化复制信息；若该类被显式设置过则跳过，返回是否由本函数完成初始化
...
// 初始化全局类设置：先应用配置里的 ClassSettings，再叠加代码里显式指定的那些
// （例如 GameState / PlayerState / Character / PlayerController 各自的路由方式）
...
// 创建全局节点：2D 网格节点 + AlwaysRelevant 节点，并读配置设好网格尺寸
...
// 为每条连接创建专属节点（AlwaysRelevant_ForConnection + PlayerState 限流节点）
...
// 推导一个类的路由策略：配置优先，其次按父类继承，最后给默认值
...
// 核心分派：按该类的策略把 Actor 放进网格节点 / AlwaysRelevant 节点 / 流关列表
...
#if WITH_GAMEPLAY_DEBUGGER
// GameplayDebugger 换了归属玩家：把它挪到对应连接的专属节点
...
// 收集本连接要复制的列表：始终相关 Actor + PlayerState（首次要单独处理）+ 可见流关里的 Actor
...
// 客户端某流关变为可见：把该流关的始终相关 Actor 加进待复制队列
...
// 每帧准备：把 PlayerState 分桶，本帧只放出其中一桶以限制复制频率
...
// 交出本帧该复制的 PlayerState（ForceNetUpdate 的永远优先）

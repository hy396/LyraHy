...
// 特性名常量：其它组件用 "PawnExtension" 声明依赖
...
// 注册：把本特性登记进 UGameFrameworkComponentManager，并声明它依赖哪些前置特性
...
// 开始：把自己注册进初始化状态机，尝试推进到 Spawned 之后的默认状态
...
// 设置 PawnData：只允许设置一次（已有了会报警），设置后立刻尝试继续推进初始化
...
// 客户端收到复制的 PawnData：同样尝试推进初始化
void ULyraPawnExtensionComponent::OnRep_PawnData()
{
	CheckDefaultInitialization();
}

// 把 Pawn 注册为 ASC 的 Avatar：设置 AvatarActor/OwnerActor，授予 PawnData 里的能力集，
// 然后广播 OnAbilitySystemInitialized
...
// 反初始化：取消已授予的能力、清掉 Avatar，广播 OnAbilitySystemUninitialized
...
// Controller 变了：更新状态机里的 Controller 就绪标记
...
// PlayerState 复制下来了：更新就绪标记，这是 ASC 可用的前提
...
// 按默认路径推进：依次尝试 DataAvailable / DataInitialized / GameplayReady
...
// 判断能否迁移：DataAvailable 要求 PawnData 已设；DataInitialized 要求 Controller 与
// PlayerState 都就绪；这是整个 Pawn 初始化的「闸门」所在
...
// 执行迁移：到 DataInitialized 时去拿 ASC 并初始化，之后放行
...
// 关心的 Actor（通常是 PlayerState）状态变了：重新尝试推进自己
...
// 注册回调；若 ASC 已经初始化过则立刻同步调用一次，避免调用方漏掉

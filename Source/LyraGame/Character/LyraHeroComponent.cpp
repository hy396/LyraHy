...
// 注册：登记本特性，并声明它依赖 PawnExtension 特性（所以它一定排在 PawnExtension 之后初始化）
...
// 判断能否迁移：核心是等 PawnExtension 到达 DataInitialized；
// 到达 GameplayReady 前还要确认输入已经绑定好
...
// 执行迁移：到 DataInitialized 时绑定输入并广播 NAME_BindInputsNow，
// 让技能/其它系统有机会在真正可用之前再挂一批输入
...
// 绑定输入：先加 PawnData 里的 InputConfig，再加组件上配的默认输入映射，
// 最后把 InputTag 与「按下/松开」回调接上
...
// 按下：把 InputTag 转成 AbilitySpec 的按下事件交给 ASC
...
// 鼠标视角：直接加 Controller 的 Yaw/Pitch
...
// 摇杆视角：按帧率归一化后加 Controller 的 Yaw/Pitch
...
// 决定摄像机模式：技能覆盖优先，否则用 PawnData 里的默认模式

...
namespace LyraAbilityUtils
{
	// EGameplayAbilityInstancingPolicy::NonInstanced is deprecated in UE 5.6, so test for the
	// instanced policies explicitly instead of comparing against it.
	bool IsAbilityInstanced(const UGameplayAbility* Ability)
	{
		if (!Ability)
		{
			return false;
		...
		const EGameplayAbilityInstancingPolicy::Type Policy = Ability->GetInstancingPolicy();
		return Policy == EGameplayAbilityInstancingPolicy::InstancedPerActor
			|| Policy == EGameplayAbilityInstancingPolicy::InstancedPerExecution;
	}

	// Instanced abilities keep their activation info on the instance itself, while the spec's
	// ActivationInfo is only meaningful for (deprecated) non-instanced abilities.
	FGameplayAbilityActivationInfo GetSpecActivationInfo(const FGameplayAbilitySpec& Spec)
	{
		if (const UGameplayAbility* PrimaryInstance = Spec.GetPrimaryInstance())
		{
			return PrimaryInstance->GetCurrentActivationInfo();
		}

PRAGMA_DISABLE_DEPRECATION_WARNINGS
		return Spec.ActivationInfo;
...
void ULyraAbilitySystemComponent::InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor)
{
	...
	if (bHasNewPawnAvatar)
	{
		...
		for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities.Items)
		{
			...
			if (LyraAbilityUtils::IsAbilityInstanced(LyraAbilityCDO))
			{
...
void ULyraAbilitySystemComponent::AbilitySpecInputPressed(FGameplayAbilitySpec& Spec)
{
	...
	if (Spec.IsActive())
	{
		...
		InvokeReplicatedEvent(EAbilityGenericReplicatedEvent::InputPressed, Spec.Handle, LyraAbilityUtils::GetSpecActivationInfo(Spec).GetActivationPredictionKey());
	}
}
...
void ULyraAbilitySystemComponent::AbilitySpecInputReleased(FGameplayAbilitySpec& Spec)
{
	...
	if (Spec.IsActive())
	{
		...
		InvokeReplicatedEvent(EAbilityGenericReplicatedEvent::InputReleased, Spec.Handle, LyraAbilityUtils::GetSpecActivationInfo(Spec).GetActivationPredictionKey());
	}
}

...
void ULyraSimulatedInputWidget::InputKeyValue(const FVector& Value)
{
	...
	else if (UEnhancedPlayerInput* Input = GetPlayerInput())
	{
		...
		{
			// UE 5.6 replaced FInputKeyParams with FInputKeyEventArgs. Use the simulated helper so the
			// event is flagged as non-physical input, and feed the 1D axis amount through AmountDepressed.
			FInputKeyEventArgs Params = FInputKeyEventArgs::CreateSimulated(
				KeyToSimulate,
				IE_Pressed,
				/*AmountDepressed*/ static_cast<float>(Value.X),

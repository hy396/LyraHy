...
// 构造：设置默认的 PlayerState / Controller / HUD 等类
...
// 取该控制器要用的 Pawn 数据：优先 PlayerState 上的 PawnData，没有则回退到体验的默认值
...
// 没有外部指定体验时自己挑一个，优先级从高到低：
//   匹配分配 > URL 参数 Experience= > PIE 开发者设置 > 命令行 Experience=
//   > 世界设置默认体验 > 专用服务器登录 > 兜底的 B_LyraDefaultExperience
...
// 专用服务器且在默认地图上时才走这里：做一次在线登录，成功后自己开房；
// 返回 true 表示已进入专用服务器流程，调用方不要再走普通体验加载
...
// 专用服务器开房：从命令行读 UserExperience=/Playlist= 决定跑哪个面向玩家的体验
...
// 体验 ID 已确定：交给 ExperienceManagerComponent 正式开始加载
...
// 体验加载完成：此时才允许玩家出生，会把之前挂着等待重生的玩家放行
...
// 取默认 Pawn 类：来自 PawnData，取不到就不生成
...
// 生成 Pawn 并在生成后立刻通知 LyraPawnExtensionComponent、绑定 PawnData
...
// Lyra 不用 StartSpot 标签定位，交给 PlayerSpawningManagerComponent 选点，所以恒为 false
...
// 新玩家加入：只做通用初始化，出生时机由体验加载完成后统一放行
...
// 选出生点：委托给 LyraPlayerSpawningManagerComponent
...
// 重生完成：通知 PlayerController 并广播玩家初始化完成
...
// 初始化 GameState：在这里创建 ExperienceManagerComponent 并挂上体验
...
// 通用玩家初始化：广播 OnGameModePlayerInitialized，供各子系统挂玩家级组件
...
// 请求下一帧重生；bForceReset 为真时立刻放弃当前 Pawn 并重置控制器

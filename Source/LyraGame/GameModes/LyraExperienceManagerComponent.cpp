...
// 构造：开启复制、设置复制间隔；组件本身是 GameState 的子对象
...
// 设置并加载体验：只在服务端真正驱动加载，客户端通过 OnRep_CurrentExperience 跟进
...
// 注册高优先级回调；已加载完就立刻同步调用，否则挂到委托上等广播
...
// 取当前体验；没加载完会直接 check 失败，所以调用前必须先 IsExperienceLoaded
...
// 客户端收到服务端复制的体验 ID，开始本地加载流程
void ULyraExperienceManagerComponent::OnRep_CurrentExperience()
{
	StartExperienceLoad();
}

// 流水线第 1 步：把体验本身 + 它引用的所有动作集加进 AssetBundle，
// 按 Client/Server 网络模式挑要加载的 Bundle，异步加载完成后进入下一步
...
// 流水线第 2 步：资产就绪，把体验与动作集里声明的插件名解析成插件 URL，
// 去重后逐个 LoadAndActivateGameFeaturePlugin；一个都没有就直接跳到第 3 步
...
// 单个插件加载完成：计数减一，全部完成才继续（这是并行加载的汇合点）
...
// 流水线第 3 步：可插入混沌测试延迟，然后按 Registering -> Loading -> Activating
// 的顺序执行所有 UGameFeatureAction，最后按 高/普通/低 三级顺序广播完成委托
...
// 一个动作反激活完成：计数减一，全部完成则进入 OnAllActionsDeactivated
...
// 复制 CurrentExperience，客户端靠它跟随服务端加载同一个体验
...
// 结束：按 Deactivating 流程把所有已执行的动作反激活，再卸载插件
...
// ILoadingProcessInterface：体验没加载完就返回 true，让加载界面一直显示

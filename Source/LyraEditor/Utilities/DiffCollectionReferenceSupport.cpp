		...
		[](const TArray<FString>& Params, UWorld* World, FOutputDevice& Ar)
{
	...
	// UE 5.6 moved collections behind an ICollectionContainer, so go through the project container.
	const TSharedRef<ICollectionContainer>& ProjectCollections = CollectionManager.GetProjectCollectionContainer();

	TArray<FSoftObjectPath> OldPaths;
	if (!ProjectCollections->GetAssetsInCollection(FName(*Params[0]), ECollectionShareType::CST_All, /*out*/ OldPaths))
	{
	...
	if (!ProjectCollections->GetAssetsInCollection(FName(*Params[1]), ECollectionShareType::CST_All, /*out*/ NewPaths))
	{

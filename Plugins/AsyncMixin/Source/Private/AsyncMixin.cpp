...
// 析构：从全局状态表里摘掉自己。这一步会自动取消所有未完成的加载，
// 从而保证之后不会再有任何回调打到已经销毁的宿主上——这正是可以放心捕获 [this] 的原因。
FAsyncMixin::~FAsyncMixin()
{
...
// 启动流水线：先回调 OnStartedLoading，再让内部状态真正把各步骤发出去。
...
// 取消：把未完成步骤的回调全部解绑，已完成的则移到"待销毁"列表里延后清理。
...
// 取消并安排销毁这份状态（下一帧真正释放，避免在当前调用栈里删自己）。
...
// 预约下一帧销毁本状态。用 Bundle 预加载时不会走到这里（句柄必须保留）。
...
// 真正开始：把已经攒好的步骤逐个发出去；若当前步骤尚未完成则停在那里等它。
...
// 插入一个"纯回调"步骤：不加载任何东西，只是占一个位置，用来保证执行顺序。
...
// 用户没显式调 StartAsyncLoading 时的兜底：安排下一帧自动启动。
...
// 整条流水线完成：回调宿主的 OnFinishedLoading，并安排销毁这份状态。

...
bool UVrmLoaderComponent::LoadVRMFileAsync(const UVrmAssetListObject *InVrmAsset, FString filepath) {
	...
	// AssetList 是 TObjectPtr，无法直接绑定到 UVrmAssetListObject*&，用局部裸指针过渡
	UVrmAssetListObject* OutVrmAsset = AssetList;
	ULoaderBPFunctionLibrary::LoadVRMFileLocal(InVrmAsset, OutVrmAsset, filepath);
	AssetList = OutVrmAsset;
	bResultQueue = true;
	return true;
}

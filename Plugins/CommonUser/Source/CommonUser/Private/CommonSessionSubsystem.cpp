...
// SEARCH_PRESENCE ("PRESENCESEARCH") is deprecated as of UE 5.5 and will eventually stop being a
// UE-defined key. The online subsystems still understand it, so declare the key locally to keep the
// exact same session/lobby behaviour without pulling in the deprecated symbol.
static const FName CommonSession_SearchPresence(TEXT("PRESENCESEARCH"));
...
public:
		...
		: FCommonOnlineSearchSettingsBase(InSearchRequest)
	{
		...
		{
			QuerySettings.Set(CommonSession_SearchPresence, true, EOnlineComparisonOp::Equals);
...
public:
		...
		: FCommonOnlineSearchSettingsBase(InSearchRequest)
	{
		...
		if (InSearchRequest->bUseLobbies)
		{
			FindLobbyParams.Filters.Emplace(FFindLobbySearchFilter{ CommonSession_SearchPresence, ESchemaAttributeComparisonOp::Equals, true });
		}
	}
public:
...
// 把 MapID（主资源 ID）解析成可供 ServerTravel 使用的完整地图名
...
// 拼装 Travel URL：地图名 + listen + ExtraArgs 中的附加参数
...
// 校验请求合法性（地图是否有效、人数是否合理等），不合法时写错误文本并打日志
...
// 子系统初始化：判断是否专用服务器、绑定在线回调并注册 Travel 失败处理
...
// 创建带默认联机参数的建房请求（bUseLobbies 取配置里的默认值）
...
// 开房主流程：校验请求 -> 建立在线会话 -> 成功后 ServerTravel 到目标地图
...
void UCommonSessionSubsystem::CreateOnlineSessionInternalOSSv2(ULocalPlayer* LocalPlayer, UCommonSession_HostSessionRequest* Request)
{
	...
	if (bIsPresence)
	{
		...
		CreateParams.Attributes.Emplace(CommonSession_SearchPresence, true);
	}
...
// 建房收尾：注册本地玩家、启动会话，然后按 PendingTravelURL 执行 Travel
...
// 快速匹配：先用建房参数构造搜索条件去找现成会话，找不到再自己建房
...
// 快速匹配搜索结束：成功且有结果就加入第一个，否则用原建房请求自己开房
...
#if COMMONUSER_OSSV1
// 搜索完成回调：把底层结果逐个包装成 UCommonSession_SearchResult 并通知请求对象
...
// 加入收尾：解析连接串、广播 PreClientTravel（可改 URL）、然后客户端 Travel
...
// 统一的 Travel 实现：主机走 ServerTravel，客户端走 ClientTravel
...
// 广播“会话可展示信息变化”事件，供 Rich Presence 等使用
...
// 玩家接受了平台邀请：把邀请对应的会话包装成搜索结果并广播请求事件
...
// 本地 Travel 失败处理：清理会话状态并把错误呈现给玩家

...
// 构造函数：所有配置字段都靠 Config 默认值填充，这里无需额外处理
UCommonUserBasicPresence::UCommonUserBasicPresence()
{

}

// 初始化：先确保会话子系统已创建，再订阅它的“会话信息变化”事件
...
// 反初始化：无需额外清理（事件绑定随对象析构自动解除）
void UCommonUserBasicPresence::Deinitialize()
{

}

// 把会话状态枚举翻译成后端约定的状态字符串；未知枚举值会打 Error 日志并返回 Unknown
...
// 会话信息变化：地图名是 URL 形式，先截掉路径只留地图短名，再逐个本地玩家推送 Presence

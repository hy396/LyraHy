...
// 取指定上下文的缓存数据（可写）；上下文为 Game 时会按解析规则找到真正的目标上下文
...
// 写入某上下文下的权限结果；若写入的是平台上下文，会把结果向 Game 上下文传播
...
// 取最近一次查询到的权限结果；从未查询过则返回 Unknown
...
// 综合“缓存的权限结果 + 当前初始化状态”得出一个可直接给 UI 用的可用性结论
...
// 取指定上下文下的 NetId，不存在时返回无效的 NetId
...
// 子系统初始化：创建在线上下文、绑定在线层回调，并为首个 LocalPlayer 建立用户信息
...
// 在绑定回调之前先把各在线上下文（默认/平台/服务）的缓存对象建好
...
// 创建一个新的 UCommonUserInfo 对象，子类可重写以使用自定义的用户信息类
...
// 绑定在线层回调的总入口，内部按 OSS 版本分派到 BindOnlineDelegatesOSSv1 / OSSv2
...
// 强制登出并反初始化单个本地用户，不销毁 LocalPlayer
...
// 流水线第 1 步：用平台层账号令牌去登录服务层；OSSv1 不支持，直接返回 false
...
// 流水线第 4 步：查询所需权限（例如能否联机），结果决定初始化是否算成功
...
// 取指定平台用户在某个上下文下的登录状态，只对真实平台用户有效
...
// 向 UI 发送一条系统消息（错误/警告弹窗），通过 OnHandleSystemMessage 广播
...
// 取指定本地玩家的初始化状态；UI 靠它决定显示登录提示还是放行进入游戏
...
// 为本地单机游玩初始化：请求 CanPlay 权限，可新建 LocalPlayer，可用访客登录
...
// 为联机游玩登录：请求 CanPlayOnline 权限，不新建 LocalPlayer
...
// 通用入口：按 Params 发起一次完整初始化；返回 false 表示流程没能启动（不会有任何回调）
...
// 注册按键监听：AnyUserKeys 用于“已有用户按任意键”，NewUserKeys 用于“新用户加入”
...
// 取消进行中的初始化；即使平台无法真正取消，也保证不再触发回调
...
// 登出指定本地玩家；bDestroyPlayer 为真时连同 LocalPlayer 一起销毁（首个玩家除外）
...
// 出错返回主菜单时调用：清空登录/初始化状态，让玩家可以重新走一遍流程
...
// 输入拦截：判断本次按键是否命中登录按键，命中则触发对应登录并吞掉该输入
...
// 登录回调转初始化：比对本次登录是否满足 Params 要求的权限与上下文
...
// 底层登录入口：对指定用户发起一次真正的在线登录，结果通过 OnComplete 返回
...
// 流水线驱动：每次调用推进一个阶段，全部阶段结束后回调一次完成委托
...
#if COMMONUSER_OSSV1
// OSSv1 登录完成回调（自动登录或平台 UI 登录均走这里）
...
// OSSv1 登录界面关闭回调：界面关闭即视为本次登录结束（可能成功也可能失败）
...
// OSSv1 权限查询完成回调：把平台返回的位标志转换成 ECommonUserPrivilegeResult
...
// 从在线层重新拉取并刷新该用户的缓存信息（NetId、昵称、登录状态）
...
#if COMMONUSER_OSSV1
// 把 OSSv1 的权限枚举转换成插件自己的 ECommonUserPrivilege
...
#else
// 把 OSSv2 的权限枚举转换成插件自己的 ECommonUserPrivilege
...
// 把 LocalPlayer 与某个用户信息绑定，并触发必要的回调
...
// 按平台用户索引取用户信息（旧接口，建议改用 PlatformUserId 版本）
...
// 按控制器 ID 取用户信息（旧接口，建议改用 InputDevice 版本）

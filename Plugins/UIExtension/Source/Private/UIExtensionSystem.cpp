...
// 句柄自身的便捷反注册：转发给它所来自的那个 Subsystem。
...
// 判定一条扩展是否满足本扩展点的"契约"。两个条件必须同时成立：
//   1) 上下文匹配：两者都为空，或指向同一个对象；
//   2) 数据类匹配：扩展的数据类（若数据本身是 UClass 就用它，否则用它的 GetClass）
//      必须继承自、或实现了 AllowedDataClasses 中的某个类。
// 不满足则这条扩展会被静默丢弃，扩展点完全收不到——这是排查"扩展没生效"时的首要检查点。
...
{
	// Chain to the parent implementation, otherwise the GC misses whatever UObject::AddReferencedObjects
	// collects (and logs "Class UIExtensionSubsystem or a super class did not call Super::AddReferencedObjects").
...
// 注销一个扩展点：先从表里摘掉（避免再收到回调），再从映射中移除。
...
// 把内部数据打包成回调参数：句柄、标签、优先级、数据、上下文对象。
